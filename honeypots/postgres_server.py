    ...
    def postgres_server_main(self):
        ...
        class CustomPostgresProtocol(Protocol):
            ...
            def connectionMade(self):
                self._state = 1
                self._variables = {}
                _q_s.logs.info(dumps({'timestamp': datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), 'protocol': 'postgres', 'action': 'connection', 'src_ip': self.transport.getPeer().host, 'src_port': self.transport.getPeer().port, 'dest_port': _q_s.port}))
