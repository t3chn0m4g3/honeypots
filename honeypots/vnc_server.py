    ...
    def vnc_server_main(self):
        ...
        class CustomVNCProtocol(Protocol):
            ...
            def connectionMade(self):
                ...
                self._state = 1
                _q_s.logs.info(dumps({'timestamp': datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), 'protocol': 'vnc', 'action': 'connection', 'src_ip': self.transport.getPeer().host, 'src_port': self.transport.getPeer().port, 'dest_port': _q_s.port}))
