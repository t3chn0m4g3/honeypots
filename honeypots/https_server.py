...
'''
            ...
            def render(self, request):
                ...
                _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "https", "action": "connection", "src_ip": request.getClientIP(), "dest_port": _q_s.port, "request": headers})
                ...
                if request.method == b"GET":
                    _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "https", "action": "get", "src_ip": request.getClientIP(), "dest_port": _q_s.port})
                    ...
                    _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "https", "action": "post", "src_ip": request.getClientIP(), "dest_port": _q_s.port})
                        ...
                        if _q_s.username != '' and _q_s.password != '':
                            ...
                            if 'username' in form and 'password' in form:
                                ...
                                if form['username'].value == _q_s.username and form['password'].value == _q_s.password:
                                    _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "https", "action": "login", "status": "success", "src_ip": request.getClientIP(), "dest_port": _q_s.port, "username": _q_s.username, "password": _q_s.password})
                                else:
                                    _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "https", "action": "login", "status": "failed", "src_ip": request.getClientIP(), "dest_port": _q_s.port, "username": form["username"].value, "password":form["password"].value})
            ...
            if auto and not self.auto_disabled:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "https", "action": "process", "status": "success", "route": "/login.html", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password})
                    else:
                        self.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "https", "action": "process", "status": "error", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password})
                else:
                    self.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "https", "action": "setup", "status": "error", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password})
