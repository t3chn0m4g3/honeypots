...
'''
                        ...
                        if _q_s.username != '' and _q_s.password != '':
                            ...
                            if 'username' in form and 'password' in form:
                                ...
                                if username == _q_s.username and password == _q_s.password:
                                    ...
                                    status = 'success'
                                _q_s.logs.info(dumps({'timestamp': datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), 'protocol': 'https', 'action': 'login', 'status': status, 'src_ip': request.getClientIP(), 'dest_port': _q_s.port, 'username': form['username'].value, 'password':form['password'].value}))
    ...
    def run_server(self, process=False, auto=False):
        ...
        if process:
            ...
            self.logs.info(dumps({'timestamp': datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), 'protocol': 'https', 'action': 'process', 'status': status, 'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password}))
