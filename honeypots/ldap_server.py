...
class QLDAPServer():
    ...
    def run_server(self, process=False, auto=False):
        ...
        if process:
            ...
            self.logs.info(dumps({'timestamp': datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), 'protocol': 'ldap', 'action': 'process', 'status': status, 'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password}))
