    ...
    def ldap_server_main(self):
        ...
        class CustomLDAProtocol(Protocol):
            ...
            def connectionMade(self):
                self._state = 1
                _q_s.logs.info(dumps({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "ldap", "action": "connection", "src_ip": self.transport.getPeer().host, "src_port": self.transport.getPeer().port, "dest_port": _q_s.port}))
                ...
                if self._state == 1:
                    ...
                    if username != "" or password != "":
                        if username == _q_s.username and password == _q_s.password:
                            _q_s.logs.info(dumps({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "ldap", "action": "login", "status": "success", "src_ip": self.transport.getPeer().host, "src_port": self.transport.getPeer().port, "dest_port": _q_s.port, "username": _q_s.username, "password": _q_s.password}))
                        else:
                            _q_s.logs.info(dumps({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "ldap", "action": "login", "status": "failed", "src_ip": self.transport.getPeer().host, "src_port": self.transport.getPeer().port, "dest_port": _q_s.port, "username": username, "password": password}))
            ...
            if auto and not self.auto_disabled:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info(dumps({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "ldap", "action": "process", "status": "success", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password}))
                    else:
                        self.logs.info(dumps({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "ldap", "action": "process", "status": "error", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password}))
                else:
                    self.logs.info(dumps({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "ldap", "action": "setup", "status": "error", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password}))
