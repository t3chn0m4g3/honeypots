...
class QSOCKS5Server():
                    ...
                    if 2 in unpack('!' + 'B' * m, self.connection.recv(m)):
                        ...
                        if 1 in unpack('B', self.connection.recv(1)):
                            ...
                            if username == _q_s.username and password == _q_s.password:
                                ...
                                status = 'success'
                            _q_s.logs.info(dumps({'timestamp': datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), 'protocol': 'socks5', 'action': 'login', 'status': status, 'src_ip': self.client_address[0], 'src_port':self.client_address[1], 'dest_port': _q_s.port, 'username':username.decode(), 'password':password.decode()}))
    ...
    def run_server(self, process=False, auto=False):
        ...
        if process:
            ...
            self.logs.info(dumps({'timestamp': datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%fZ'), 'protocol': 'socks5', 'action': 'process', 'status': status, 'ip': self.ip, 'port': self.port, 'username': self.username, 'password': self.password}))
