...
class QSOCKS5Server():
    ...
    def socks5_server_main(self):
        ...
        class CustomStreamRequestHandler(StreamRequestHandler):
            ...
            def handle(self):
                _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "socks5", "action": "connection", "src_ip": self.client_address[0], "src_port":self.client_address[1], "dest_port": _q_s.port})
                    ...
                    if 2 in unpack("!" + "B" * m, self.connection.recv(m)):
                        ...
                        if 1 in unpack("B", self.connection.recv(1)):
                            ...
                            if username == _q_s.username and password == _q_s.password:
                                _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "socks5", "action": "login", "status": "success", "src_ip": self.client_address[0], "src_port":self.client_address[1], "dest_port": _q_s.port, "username":_q_s.username, "password":_q_s.password})
                            else:
                                _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "socks5", "action": "login", "status": "failed", "src_ip": self.client_address[0], "src_port":self.client_address[1], "dest_port": _q_s.port, "username":username.decode(), "password":password.decode()})
            ...
            if auto and not self.auto_disabled:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "socks5", "action": "process", "status": "success", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password})
                    else:
                        self.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "socks5", "action": "process", "status": "error", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password})
                else:
                    self.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "socks5", "action": "setup", "status": "error", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password})
