    ...
    def mysql_server_main(self):
        ...
        class CustomMysqlProtocol(Protocol):
            ...
            def connectionMade(self):
                ...
                _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "mysql", "action": "connection", "src_ip": self.transport.getPeer().host, "src_port": self.transport.getPeer().port, "dest_port": _q_s.port})
                    ...
                    if good:
                        if password:
                            ...
                            if _x == _q_s.password and _x is not None:
                                _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "mysql", "action": "login", "status": "success", "src_ip": self.transport.getPeer().host, "src_port": self.transport.getPeer().port, "dest_port": _q_s.port, "username": _q_s.username, "password": _q_s.password})
                            else:
                                _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "mysql", "action": "login", "status": "failed", "src_ip": self.transport.getPeer().host, "src_port": self.transport.getPeer().port, "dest_port": _q_s.port, "username": username, "password": ":".join(hex((c))[2:] for c in password)})
                        else:
                            _q_s.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "mysql", "action": "login", "status": "failed", "src_ip": self.transport.getPeer().host, "src_port": self.transport.getPeer().port, "dest_port": _q_s.port, "username": "UnKnown", "password": ":".join(hex((c))[2:] for c in data)})
            ...
            if auto and not self.auto_disabled:
                ...
                if port > 0:
                    ...
                    if self.process.poll() is None:
                        self.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "mysql", "action": "process", "status": "success", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password})
                    else:
                        self.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "mysql", "action": "process", "status": "error", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password})
                else:
                    self.logs.info({"timestamp": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "protocol": "mysql", "action": "setup", "status": "error", "ip": self.ip, "port": self.port, "username": self.username, "password": self.password})
